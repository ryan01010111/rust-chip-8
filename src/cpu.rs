...
impl Cpu {
    ...
    pub fn load_rom(&mut self, path: std::path::PathBuf) -> Result<(), io::Error> {
        ...
        self.memory[start_addr..start_addr + file_bytes.len()]
    ...
    fn cycle(&mut self) -> Result<(), io::Error> {
        ...
        if !self.paused { self.update_timers(); }
        ...
        if !timeout.is_zero() { std::thread::sleep(timeout); }

        Ok(())
    }

    fn process_next_key(&mut self) {
        let params = self.next_key_params.as_ref()
        ...
        if let Some(val) = self.keyboard.get_next_key(params.valid_after) {
            if self.keyboard.esc_pressed || self.keyboard.pause_toggle_on { return; }
    ...
    fn exec_instruction(&mut self, opcode: u16) {
        ...
        match opcode & 0xF000 {
            ...
            0x3000 => { // SE Vx, byte
                ...
                if self.v[x] == opcode as u8 { self.pc += 2; }
            }
            0x4000 => { // SNE Vx, byte
                // Skip next instruction if Vx != kk.
            ...
            0xD000 => { // DRW Vx, Vy, nibble
                ...
                // stored in I. These bytes are then displayed as sprites on screen at coordinates
                // (Vx, Vy). Sprites are XORed onto the existing screen. If this causes any pixels
                ...
                for row in 0..cmp::min(sprite_byte_len, max_height) {
                    ...
                    for col in 0..cmp::min(8, max_width) {
                        ...
                        if sprite_row & 0x80 > 0 {
                            let has_collision = self.display
                                .set_block(x_start + col, y_start + row);
                            if has_collision { self.v[0xF] = 1; }
                        }
            ...
            0xF000 =>
                match opcode & 0xFF {
                    ...
                    0x55 => { // LD [I], Vx
                        ...
                        let start_addr = self.i as usize;
                        self.memory[start_addr..=start_addr + x]
                            .copy_from_slice(&self.v[0x0..=x])
                    }
                    0x65 => { // LD Vx, [I]
